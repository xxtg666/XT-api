    ...
    if (module_name.endswith(".py") or os.path.isdir("modules/"+module_name)) and not module_name.startswith("_"):
        ...
        try:
            ...
            for res in module.resources:
                ep = "".join(random.choices("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789", k=20))
                api.add_resource(getattr(module,res["name"]), '/'+module_name+res["args"],endpoint=ep)
                l.success("模块 "+module_name+"."+res["name"]+" 载入成功"+" (ep: "+ep+")")
        except:
